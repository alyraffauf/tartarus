...
rec {
  ...
  edit =
    { pkgs, ... }:
    let
      editFile = pkgs.writers.writePython3Bin "edit-file" { flakeIgnore = [ "E501" ]; } ''
        import pathlib
        import sys
  ...
  glob =
    { pkgs, ... }:
    let
      globFiles = pkgs.writers.writePython3Bin "glob-files" { flakeIgnore = [ "E501" ]; } ''
        import pathlib
        import sys
